...
struct h_node make_branch(struct h_node *node1, struct h_node *node2) {
   if (n_comes_before(node1, node2)) {
      if (node1->character < node2->character) {
         return make_node(node1->occurence + node2->occurence, node1->character, node1, node2);
      } else {
         return make_node(node1->occurence + node2->occurence, node2->character, node1, node2);
      }
   } else {
       if (node1->character < node2->character) {
         return make_node(node1->occurence + node2->occurence, node1->character, node2, node1);
      } else {
         return make_node(node1->occurence + node2->occurence, node2->character, node2, node1);
      }
   }
}
